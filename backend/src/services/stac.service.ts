...
export async function getCollections(): Promise<Collection[]> {
  try {
    ...
    const data = res.data as { collections: any[] };
...
export async function searchItemsByPoint(
  ...
  const payload: any = {
    ...
    datetime: `${startDate || config.defaults.startDate}/${
  ...
  try {
    ...
    const data = res.data as { features?: any[] };
    console.log(`STAC retornou ${data.features?.length || 0} itens`);
